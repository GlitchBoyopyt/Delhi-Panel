...
router.post('/instance/reinstall/:id', async (req, res) => {
    ...
    try {
        ...
        const { Node: node, Image: shortimage, Memory: memory, Cpu: cpu, Ports: ports, Name: name, User: user, Primary: primary, ContainerId: containerId, Env } = instance;

        if (!shortimage || !memory || !cpu || !ports || !node || !name || !user || !primary) {
        ...
        console.error('[Delhi Panel] Error reinstalling instance:', error);
        res.status(500).json({
