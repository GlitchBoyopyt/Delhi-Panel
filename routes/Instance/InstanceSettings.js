...
router.get("/instance/:id/change/name/:name", async (req, res) => {
    ...
    if (!/^[a-zA-Z0-9 ]+$/.test(name)) return res.status(400).send('Invalid name format');
    ...
    // Update instance in user's instances list
    ...
    if (userInstanceIndex !== -1) {
        userInstances[userInstanceIndex].Name = instance.Name;
        await db.set(`${req.user.userId}_instances`, userInstances);
    }

    // Update instance in global instances list
    ...
    if (globalInstanceIndex !== -1) {
        globalInstances[globalInstanceIndex].Name = instance.Name;
